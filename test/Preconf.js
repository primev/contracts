...
describe("Preconf", function () {
  ...
  describe("Deployment", function () {
    it("Should deploy the smart contract and confirm preconf signature validity & bid validity & ensure both user and provider that are part of the preconf have staked eth", async function () {
      ...
      const providerRegistry = await ethers.deployContract("ProviderRegistry", [ethers.parseEther("2.0")]);
      await providerRegistry.waitForDeployment();


      const userRegistry = await ethers.deployContract("UserRegistry", [ethers.parseEther("2.0")]);
      ...
      const preconf = await ethers.deployContract("PreConfCommitmentStore", [providerRegistry.target, userRegistry.target, oracle]);
  ...
  it("Reward a provider via the oracle into the preconf contract", async function () {
    ...
    await addr1.sendTransaction({to: bidderAddress, value: ethers.parseEther("50.0")});
    await addr1.sendTransaction({to: commiterAddress, value: ethers.parseEther("50.0")});
    ...
    */
    ...
    const txnSlash = await preconf.connect(oracle).initateReward(commitmentHash);
    await txnSlash.wait();
  });
  ...
  it("Slash a provider via the oracle into the preconf contract", async function () {
    ...
    */
    ...
    const txnSlash = await preconf.connect(oracle).initiateSlash(commitmentHash);
    await txnSlash.wait();
  });
});
